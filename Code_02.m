...
for i=1:1:200
   for j=1:1:400
       if ((i-50)^2+(j-250)^2)<1600
           AgM(i,j)=-2;
       end
           if ((i-50)^2+(j-150)^2)<1600||((i-150)^2+(j-200)^2)<1600
          AgM(i,j)=-1;
       end
   end
end
...
for i=1:1:200
    for j=1:1:400
        Mo=Mo+1;
            if hopp(i,j)<(Pdl(i,j)*dt)&&AgM(i,j)>0
            AgM(i,j)=AgM(i,j)-1;
            AgM(i+1,j-1)=1;
        else
            if hopp(i,j)<(Pdl(i,j)*dt+Pu(i,j)*dt)&&AgM(i,j)>0
            AgM(i,j)=AgM(i,j)-1;
            AgM(i-1,j)=1;
            else
             if hopp(i,j)<(Pdl(i,j)*dt+Pu(i,j)*dt+Pur(i,j)*dt)&&AgM(i,j)>0
            AgM(i,j)=AgM(i,j)-1;
            AgM(i-1,j+1)=1;
             else
               if hopp(i,j)<(Pdl(i,j)*dt+Pu(i,j)*dt+Pur(i,j)*dt+Pl(i,j)*dt)&&AgM(i,j)>0
            AgM(i,j)=AgM(i,j)-1;
            AgM(i,j-1)=1;
               else
              if hopp(i,j)<(Pdl(i,j)*dt+Pu(i,j)*dt+Pur(i,j)*dt+Pl(i,j)*dt+Pr(i,j)*dt)&&AgM(i,j)>0
            AgM(i,j)=AgM(i,j)-1;
            AgM(i,j+1)=1;
              else
